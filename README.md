...
Once the runtime starts, you can access the project at [http://localhost:8080/PetCare](http://localhost:8080/PetCare).
...
```
./mvnw clean package
docker build -t PetCare:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 PetCare:v1
```
